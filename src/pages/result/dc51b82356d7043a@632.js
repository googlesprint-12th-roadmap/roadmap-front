...
function _createChart(d3, height, width, data) {
  return function createChart(parent) {
    const svg = d3
      .select(parent)
      .append('svg')
      .attr('width', '100%')
...
function _data(transformData, roadmapper, config) {
  return transformData(roadmapper(config.children));
}

function _roadmapper(width, getRoadmapCurve) {
  return function roadmapper(data, parent) {
...
export default function define(runtime, observer) {
  ...
  main
    ...
    .define('chart', ['DOM', 'createChart'], _chart);
  main
    .variable(observer('createChart'))
    .define('createChart', ['d3', 'height', 'width', 'data'], _createChart);
  main.variable(observer('config')).define('config', _config);
  main
    .variable(observer('data'))
    .define('data', ['transformData', 'roadmapper', 'config'], _data);
  main
    .variable(observer('roadmapper'))
    .define('roadmapper', ['width', 'getRoadmapCurve'], _roadmapper);
  main
    .variable(observer('transformData'))
    .define('transformData', _transformData);
  main
    .variable(observer('getRoadmapCurve'))
    .define('getRoadmapCurve', _getRoadmapCurve);
  main.variable(observer('line')).define('line', ['d3'], _line);
  main
    .variable(observer('myCSSStyle'))
    .define('myCSSStyle', ['html'], _myCSSStyle);
